...
2. Add a new theme block in `src/styles/themes/tokens.css` keyed by the same theme identifier (using `[data-theme-astro="<theme>"]`).  
...
sequenceDiagram
  ...
  Eng->>Tokens: Add([data-theme-astro="<key>"] tokens)
...
- **Theme key mismatch**: If `config.theme` doesn’t match a `[data-theme-astro="..."]` block, components will fall back to `:root` token defaults.  
